def calculate_ckvn(data):
    ...
    for d in data:
        ...
        if d.get("macd") and d.get("macd_signal") and float(d["macd"]) < float(d["macd_signal"]) and float(d["macd"]) < 0:
...
def calculate_coin(data):
    ...
    for d in data:
        ...
        # Display rsi when vol > 1,2
        if d.get("volume_ratio") and float(d["volume_ratio"]) > 1.5 and float(d["volume_ratio"]) < 3:
            d["vol_high"] = d["volume_ratio"]
        elif d.get("volume_ratio") and float(d["volume_ratio"]) > 3:
            d["vol_high"] =  d["volume_ratio"]
        ...
        if d.get("macd") and d.get("macd_signal") and float(d["macd"]) < float(d["macd_signal"]) and float(d["macd"]) < 0:
            d["macd_down"] = "down_trend"
        elif d.get("macd") and d.get("macd_signal") and float(d["macd"]) > float(d["macd_signal"]) and float(d["macd"]) > 0:
